...
int main()
{
	...
	cudaMallocManaged(&particles_gpu, PARTICLES * sizeof(Particle));

	for (int i = 0; i < PARTICLES; i++)
	{
		particles_gpu[i].position = make_float3(rand() % 100, rand() % 100, rand() % 100);
		particles_gpu[i].velocity = make_float3(rand() % 100, rand() % 100, rand() % 100);
	}
	...
	for (int i = 0; i < ITTERATIONS; i++)
	{
		update_cpu(PARTICLES, particles_gpu, 1);
	}
	end = clock();
