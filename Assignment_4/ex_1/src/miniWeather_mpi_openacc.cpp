...
void semi_discrete_step( double *state_init , double *state_forcing , double *state_out , double dt , int dir , double *flux , double *tend ) {
  ...
  #pragma acc data copy(state_init[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS], tend[0:nx*nz*NUM_VARS], state_out[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS])
  {
...
void compute_tendencies_x( double *state , double *flux , double *tend ) {
  ...
  #pragma acc data copy(state[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS], vals, d3_vals, stencil, tend[0:nx*nz*NUM_VARS], flux[0:(nx+1)*(nz+1)*NUM_VARS])
  {
...
void compute_tendencies_z( double *state , double *flux , double *tend ) {
  ...
  //Compute fluxes in the x-direction for each cell
  #pragma acc parallel loop collapse(4) private(inds, r, u, w, t, p) copy(state[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS], vals, d3_vals, stencil, flux[0:(nx+1)*(nz+1)*NUM_VARS])
  for (k=0; k<nz+1; k++) {
    for (i=0; i<nx; i++) {
      //Use fourth-order interpolation from four cell averages to compute the value at the interface in question
      for (ll=0; ll<NUM_VARS; ll++) {
  ...
  //Use the fluxes to compute tendencies for each cell
  #pragma acc parallel loop collapse(3) private(indt, indf1, indf2, inds) copy(tend[:nx*nz*NUM_VARS], state[:(nx+2*hs)*(nz+2*hs)*NUM_VARS]) copyin(flux[:(nx+1)*(nz+1)*NUM_VARS])
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
      for (i=0; i<nx; i++) {
        indt  = ll* nz   * nx    + k* nx    + i  ;
...
void set_halo_values_x( double *state ) {
  ...
  #pragma acc parallel loop collapse(3) copy(state[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS],sendbuf_l[0:hs*nz*NUM_VARS],sendbuf_r[0:hs*nz*NUM_VARS])
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
      for (s=0; s<hs; s++) {
  ...
  #pragma acc parallel loop collapse(3) copy(recvbuf_l[0:hs*nz*NUM_VARS],recvbuf_r[0:hs*nz*NUM_VARS],state[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS])
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
      for (s=0; s<hs; s++) {
  ...
  if (data_spec_int == DATA_SPEC_INJECTION) {
    if (myrank == 0) {
      #pragma acc parallel loop collapse(2) copyin(hy_dens_cell[0:(nz+2*hs)],hy_dens_theta_cell[0:(nz+2*hs)]) copy(state[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS]) private(z, ind_r, ind_u, ind_t)
      for (k=0; k<nz; k++) {
        for (i=0; i<hs; i++) {
          z = (k_beg + k+0.5)*dz;
...
void set_halo_values_z( double *state ) {
  ...
  #pragma acc parallel loop collapse(2) copy(state[0:(nx+2*hs)*(nz+2*hs)*NUM_VARS]) private(x, xloc, mnt_deriv)
  for (ll=0; ll<NUM_VARS; ll++) {
    for (i=0; i<nx+2*hs; i++) {
      if (ll == ID_WMOM) {
